...
namespace Grading_system
{
    public partial class admin : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                error = error + 1;
                lberror4.Text = "Department can't be blank";
            }
            else
                lberror4.Text = "";
            ...
            {
                error = error + 1;
                lberror5.Text = "Email can't be blank";
            }
            {
        ...
        private void lberror4_Click(object sender, EventArgs e)
        {
        ...
        private void errorb2_Click(object sender, EventArgs e)
        {
        ...
        private void dataGridView4_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                coursetbx3.Text = row.Cells["CourseID2"].Value.ToString();

                teachertbx2.Text = row.Cells["TeacherID2"].Value.ToString();
        ...
        {
            int error = 0;
            string courseid = coursetbx3.Text;
            if (courseid.Equals(""))
            {
                error = error + 1;
                erorlb1.Text = "ID can't be blank";
            }
            else
                erorlb1.Text = "";
            string teacherid = teachertbx2.Text;
            if (teacherid.Equals(""))
            {
                error = error + 1;
                erorlb2.Text = "ID can't be blank";
            }


            {
                string query = "Select * from TeacherCourse where CourseID = @CourseID";
                conn.Open();
                SqlCommand cmdcheck = new SqlCommand(query, conn);
                cmdcheck.Parameters.Add("@CourseID", SqlDbType.NVarChar, 10);
                cmdcheck.Parameters["@CourseID"].Value = courseid;
                ...
                {
                    error++;
                    erorlb1.Text = "ID already exist, please choose another";
            ...
            if (error == 0)
            {
                string insert = "insert into TeacherCourse values (@CourseID,@TeacherID)";
                conn.Open();
                SqlCommand cmd = new SqlCommand(insert, conn);
                cmd.Parameters.Add("@CourseID", SqlDbType.VarChar);
                cmd.Parameters["@CourseID"].Value = courseid;
        ...
        {
            conn.Open();
            string deleteQuery = "DELETE FROM TeacherCourse WHERE CourseID = @CourseID";

            SqlCommand cmd = new SqlCommand(deleteQuery, conn);
            cmd.Parameters.AddWithValue("@CourseID", coursetbx3.Text);
            cmd.ExecuteNonQuery();
        ...
        {
            int error = 0;
            string courseid = coursetbx4.Text;
            if (courseid.Equals(""))
            {
                error = error + 1;
                eorlb1.Text = "ID can't be blank";
            }
            else
                eorlb1.Text = "";
            string studentid = studenttbx2.Text;
            if (studentid.Equals(""))
            {
                error = error + 1;
                eorlb2.Text = "Name can't be blank";
            }
            else
                eorlb2.Text = "";
            ...
            {
                error = error + 1;
                eorlb3.Text = "Grade can't be blank";
            }
...

            {
                ...
                {
                    error++;
                    eorlb1.Text = "Both IDs already exist, please choose another";
                    eorlb2.Text = "Both IDs already exist, please choose another";
                }
                else
                {
                    eorlb1.Text = "";
            ...
            if (error == 0)
            {
                string insert = "insert into Assignment values (@CourseID,@StudentID,@AssignmentGrade)";
                conn.Open();
                SqlCommand cmd = new SqlCommand(insert, conn);
                cmd.Parameters.Add("@CourseID", SqlDbType.VarChar);
                cmd.Parameters["@CourseID"].Value = courseid;
        ...
        private void button13_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(this, "Do you want to edit?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                SqlTransaction transaction = conn.BeginTransaction();
                SqlCommand cmd = new SqlCommand(update, conn, transaction);
                cmd.Parameters.Add("@AssignmentGrade", SqlDbType.NVarChar, 10).Value = gradetbx.Text;
                cmd.Parameters.Add("@CourseID", SqlDbType.VarChar).Value = coursetbx4.Text;
                cmd.Parameters.Add("@StudentID", SqlDbType.VarChar).Value = studenttbx2.Text;
        ...
        {
            conn.Open();
            string deleteQuery = "DELETE FROM Assignment WHERE CourseID = @CourseID";

            SqlCommand cmd = new SqlCommand(deleteQuery, conn);
            cmd.Parameters.AddWithValue("@CourseID", coursetbx4.Text);
            cmd.ExecuteNonQuery();
        ...
        private void dataGridView5_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                coursetbx4.Text = row.Cells["CourseID3"].Value.ToString();

                studenttbx2.Text = row.Cells["StudentID2"].Value.ToString();
